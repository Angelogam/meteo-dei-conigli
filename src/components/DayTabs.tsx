...
export function DayTabs({ days }: DayTabsProps) {
  ...
  // Grafici: se c'è un'ora selezionata usa quella, altrimenti la migliore del giorno
  const chartHour = displayHour;
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          {selectedHour && (
            <span className="text-[11px] text-white/30">
              Ora: <strong className="text-white/60">{formatHour(selectedHour)}</strong>
            </span>
          )}
        </div>
        <div className="flex gap-1.5">
          {currentDay?.hours.map((h) => {
            ...
            const isMiddle = !selectedHour && h.hour === chartHour?.hour;
            ...
            return (
              ...
              >
                ...
                <div
                  className="text-[9px] font-bold mt-0.5 font-mono"
                  style={{ color }}
                >
                  {h.qualityScore}
                </div>
      ...
      {chartHour && (
        <div className="mb-4 p-4 rounded-xl bg-white/[0.02] border border-white/[0.06]">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-white">{formatHour(chartHour.hour)}</span>
            <div
              className="ml-auto w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-bold"
              style={{ backgroundColor: `${getQualityColor(chartHour.qualityScore)}15`, color: getQualityColor(chartHour.qualityScore) }}
            >
              {chartHour.qualityScore}
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            <div className="p-2.5 rounded-lg bg-white/[0.03]">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] text-white/30 uppercase">Temp</span>
              </div>
              <span className="text-sm font-semibold text-white/80">{chartHour.temp.toFixed(0)}°C</span>
            </div>
            <div className="p-2.5 rounded-lg bg-white/[0.03]">
              <div className="flex items-center gap-1.5 mb-1">
                <Wind size={11} className="text-[#4DA3FF]" />
                <span className="text-[10px] text-white/30 uppercase">Vento suolo</span>
              </div>
              <span className="text-sm font-semibold text-[#4DA3FF]">{chartHour.windSpeed10m.toFixed(0)} km/h</span>
            </div>
            <div className="p-2.5 rounded-lg bg-white/[0.03]">
              <div className="flex items-center gap-1.5 mb-1">
                <ArrowUpRight size={11} className="text-[#FFC857]" />
                <span className="text-[10px] text-white/30 uppercase">Termiche</span>
              </div>
              <span className="text-sm font-semibold text-[#FFC857]">{chartHour.thermalStrength.toFixed(1)} m/s</span>
            </div>
            <div className="p-2.5 rounded-lg bg-white/[0.03]">
              <div className="flex items-center gap-1.5 mb-1">
                <Cloud size={11} className="text-white/30" />
                <span className="text-[10px] text-white/30 uppercase">Nuvole</span>
              </div>
              <span className="text-sm font-semibold text-white/80">{chartHour.cloudCover.toFixed(0)}%</span>
            </div>
            <div className="p-2.5 rounded-lg bg-white/[0.03]">
              <div className="flex items-center gap-1.5 mb-1">
                <AlertTriangle size={11} className="text-[#FF9F1C]" />
                <span className="text-[10px] text-white/30 uppercase">Turbolenza</span>
              </div>
              <span className="text-sm font-semibold text-[#FF9F1C]">{chartHour.turbulence}/5</span>
            </div>
            <div className="p-2.5 rounded-lg bg-white/[0.03]">
              <div className="flex items-center gap-1.5 mb-1">
                <Wind size={11} className="text-white/30" />
                <span className="text-[10px] text-white/30 uppercase">Umidità</span>
              </div>
              <span className="text-sm font-semibold text-white/80">{chartHour.humidity.toFixed(0)}%</span>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Vento in quota mini per l'ora selezionata */}
      {chartHour && (
        <div className="mt-4 mb-4 p-4 rounded-xl bg-white/[0.02] border border-white/[0.06]">
          <h4 className="text-xs font-semibold text-white/50 uppercase tracking-widest mb-3">
            <Wind size={12} className="inline mr-1.5" />
            Vento in quota
          </h4>
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="text-center p-2 rounded-lg bg-white/[0.03]">
              <div className="text-white/40 mb-1">1.500 m</div>
              <div className="font-semibold text-white/70">{chartHour.windSpeed1500m.toFixed(0)} km/h</div>
              <div className="text-[10px] text-white/40">{windDegToDirection(chartHour.windDirection1500m)}</div>
            </div>
            <div className="text-center p-2 rounded-lg bg-white/[0.03]">
              <div className="text-white/40 mb-1">2.500 m</div>
              <div className="font-semibold text-white/70">{chartHour.windSpeed2500m.toFixed(0)} km/h</div>
              <div className="text-[10px] text-white/40">{windDegToDirection(chartHour.windDirection2500m)}</div>
            </div>
            <div className="text-center p-2 rounded-lg bg-white/[0.03]">
              <div className="text-white/40 mb-1">3.500 m</div>
              <div className="font-semibold text-white/70">{chartHour.windSpeed3500m.toFixed(0)} km/h</div>
              <div className="text-[10px] text-white/40">{windDegToDirection(chartHour.windDirection3500m)}</div>
            </div>
          </div>
        </div>
      )}
