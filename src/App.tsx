...
function DailyCard({ day, isFirst }: { day: DailyForecast; isFirst: boolean }) {
  return (
    <div className="space-y-3">
      {/* riga superiore: icona + temperatura */}
        ...
        <div className="bg-zinc-800/60 rounded-xl p-3 space-y-2">
          ...
          <span
            className={cn(
              ...
              day.thermalQuality >= 3
                ? "text-emerald-400"
                : day.thermalQuality >= 1
                  ? "text-amber-400"
                  : "text-zinc-500"
            )}
          >
      ...
      <div className="space-y-1.5">
        ...
        {day.tips.length === 0 && (
          <p className="text-xs text-zinc-600 italic">Nessun consiglio particolare</p>
        )}
      </div>
...
export default function App() {
  ...
  const selectedLaunchData: Launch | undefined = launches.find(
    (l) => l.name === selectedLaunch
  );
  ...
  const tabIndex = useMemo(
    () => (activeTab === "oggi" ? 0 : activeTab === "domani" ? 1 : 2),
    [activeTab]
  );
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Indicatore di aggiornamento silenzioso */}
        {loading && forecast && (
      ...
      <footer className="max-w-2xl mx-auto px-4 py-6 text-center text-xs text-zinc-700">
        <p>
          Dati: Open-Meteo &bull; Algoritmi termici stimati &bull; Vola sempre
          in sicurezza
        </p>
      </footer>
    </div>
