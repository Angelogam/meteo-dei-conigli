...
import { Loader2, MapPin, Wind, Thermometer, Droplets, Cloud, Eye, TrendingUp, Mountain, ChevronDown, AlertTriangle } from "lucide-react";
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-zinc-800/60 rounded-xl p-3 space-y-2">
          ...
          {day.gusts && (
            <span className="text-xs text-amber-400">Raffiche: {day.gusts} km/h</span>
          )}
        </div>

        <div className="bg-zinc-800/60 rounded-xl p-3 space-y-2">
          <WeatherBadge icon={<Thermometer size={14} />} label="Termiche" />
          <span className={cn("text-lg font-semibold", day.thermalQuality >= 3 ? "text-emerald-400" : day.thermalQuality >= 1 ? "text-amber-400" : "text-zinc-500")}>
            {day.thermalLabel}
          </span>
      ...
      <div className="bg-zinc-800/40 rounded-xl p-3 flex items-center justify-between">
        ...
        <span className={cn("text-sm font-semibold", day.xcScore >= 3 ? "text-emerald-400" : day.xcScore >= 1 ? "text-amber-400" : "text-zinc-500")}>
          {day.xcLabel}
        </span>
      </div>
