...
import {
  ...
  getWeatherIcon, getWeatherDescription, getCloudDescription,
...
import {
  ...
  ChevronRight, Search, Loader2, Info, Compass, MapPin,
  TrendingUp, ArrowUpRight, Navigation, Clock, Gauge, Droplets,
  Mountain, Star,
} from "lucide-react";
...
export default function App() {
  ...
  const sortedSites = useMemo(() => {
    return launches.map(l => {
      ...
      const noon = central.find(h => parseInt(h.time.slice(11, 13)) === 13) || central[0];
      ...
      const thermals = noon ? noon.temperature2m : 20;
      return { launch: l, score: score.score, thermals, rating: score.rating };
    })
  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        padding: '16px 0', marginBottom: 20, borderBottom: '1px solid rgba(255,255,255,0.06)',
        flexWrap: 'wrap', gap: 12,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{
            width: 44, height: 44, borderRadius: 14,
            background: 'linear-gradient(135deg, #00FF8C20, #00FF8C08)',
            border: '1px solid #00FF8C25',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '1.4rem',
          }}>🪁</div>
          <div>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#fff', letterSpacing: '-0.02em', margin: 0 }}>
              Coniglio<span style={{ color: '#00FF8C' }}>Volo</span>
            </h1>
            <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.35)', margin: '2px 0 0' }}>
              Previsioni per volo libero · {launches.length} decolli
            </p>
          </div>
        </div>
        {!loading && (
            ...
            <div style={{
              ...
              <input
                ...
                style={{
                  ...
                  color: 'rgba(255,255,255,0.8)', fontSize: '0.8rem', width: '100%',
                  fontFamily: 'inherit',
                }}
              />
            </div>
            ...
            {currentDay && currentHourData && (
              <>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                  {processedDays.map((day, i) => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: '0.7rem', fontWeight: 700, fontFamily: 'monospace', color: day.flightScore.color, marginTop: 2 }}>
                        {day.flightScore.score}
                      </div>
                    </button>
                  ))}
                </div>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            border: `1px solid ${isActive ? `${thermal.quality >= 4 ? '#00FF8C' : thermal.quality >= 3 ? '#FFC857' : '#FF4E4E'}50` : 'transparent'}`,
                            background: isActive ? `${thermal.quality >= 4 ? '#00FF8C' : thermal.quality >= 3 ? '#FFC857' : '#FF4E4E'}15` : 'rgba(255,255,255,0.03)',
                            cursor: 'pointer',
                        ...
                        >
                          ...
                          <div style={{ width: 4, height: 4, borderRadius: '50%', margin: '2px auto 0', background: thermal.quality >= 4 ? '#00FF8C' : thermal.quality >= 3 ? '#FFC857' : '#FF4E4E' }} />
                        </button>
                      );
                    })}
                  </div>
                  ...
                  {[
                    ...
                    <div key={i} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                        {m.icon}
                        <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{m.label}</span>
                      </div>
                ...
                <div style={{
                  ...
                  background: 'rgba(79,195,247,0.03)', border: '1px solid rgba(79,195,247,0.1)',
                }}>
                  <h3 style={{ fontSize: '0.8rem', fontWeight: 600, color: '#4FC3F7', margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Wind size={14} /> Profilo vento
                  </h3>
                  {windProfile.map((level, i) => {
                    ...
                    return (
                      <div key={level.alt} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                        <div style={{ width: 60, fontSize: '0.65rem', color: 'rgba(255,255,255,0.4)', textAlign: 'right', flexShrink: 0 }}>
                          {level.alt >= 1000 ? `${(level.alt / 1000).toFixed(1)} km` : `${level.alt} m`}
                        </div>
                        <div style={{ fontSize: '1rem', width: 20, textAlign: 'center', flexShrink: 0 }}>
                          {windDegToArrow(level.dir)}
                        </div>
                        ...
                        <div style={{ width: 50, textAlign: 'right', fontSize: '0.75rem', fontWeight: 600, fontFamily: 'monospace', color, flexShrink: 0 }}>
                          {level.speed} km/h
                        </div>
                      </div>
                    );
                ...
                <div style={{
                  padding: '14px 16px', borderRadius: 14, marginBottom: 14,
                  background: 'linear-gradient(135deg, rgba(0,255,140,0.04), rgba(77,163,255,0.04))',
                  border: '1px solid rgba(0,255,140,0.1)',
                }}>
                  <h3 style={{ fontSize: '0.8rem', fontWeight: 600, color: '#00FF8C', margin: '0 0 8px', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Info size={14} /> Consigli di volo
                  </h3>
                ...
                {/* Footer */}
                <div style={{ textAlign: 'center', padding: '10px 0', fontSize: '0.6rem', color: 'rgba(255,255,255,0.2)' }}>
                  Dati: Open-Meteo · Aggiornamento ogni ora · Altitudini interpolate
                </div>
              </>
            )}
          </div>
